...
![Sequence Diagram 2 - Register New Reservation](../diagrams/sequence-02-register-reservation.png)

![Sequence Diagram 3 - Calculate and Print Bill](../diagrams/sequence-03-calculate-print-bill.png)
...
below are captured live from the running system (`docs/SETUP.md` §7),
logged in as `admin`, with real seeded data (2 reservations, 1 settled bill).

![Login page](../testing/screenshots/01-login.png)

![Dashboard with live KPIs](../testing/screenshots/03-dashboard.png)

![Register New Reservation form](../testing/screenshots/04-register-reservation-form.png)

![Reservation detail page](../testing/screenshots/05-reservation-detail.png)

![Printable bill/receipt, standard pricing strategy, marked PAID](../testing/screenshots/06-billing-receipt.png)

![Fleet & vehicle category management (Admin)](../testing/screenshots/07-vehicles.png)
...
Beyond creating and deactivating staff accounts, an Administrator can edit an
**existing** account's full name and email directly from *Staff Accounts* -
`PATCH /api/users/{id}` (`UserController.update()` → `UserServiceImpl.update()`,
validated by `UpdateUserRequest`). The edit is deliberately scoped to just
...
inherits the controller's class-level `@PreAuthorize("hasRole('ADMIN')")`, so
a `STAFF` account attempting the same request is rejected with `403
Forbidden` - verified directly in `UserServiceImplTest` and live against the
running system (`testing/TEST_CASES.md`, USER-01 to USER-06).
...
creating a new one), and `diagrams/class-diagram.mmd` now shows the
`UserService`/`UserServiceImpl` pair alongside `ReservationService` and
`BillService`, with its `update(Long, UpdateUserRequest)` method.
...
shows `Tests run: 25, Failures: 0, Errors: 0` across all nine test classes,
confirmed visually below per the Excellent-band's "screen-grabbing" requirement:

![`./mvnw test` - all 23 tests passing, BUILD SUCCESS](../testing/screenshots/14-tests-passing.png)
...
The suite is honestly reported as fully passing (25/25), but it did not start
...
history on GitHub rather than a one-off upload.

![GitHub repository - file tree, README, commit history](../testing/screenshots/11-github-repo.png)

![GitHub Actions - Backend CI run history, including the one real failure diagnosed and fixed in the very next commit (§1.1 of docs/GIT_WORKFLOW.md)](../testing/screenshots/13-github-actions.png)
...
dashboard, register-reservation, reservation detail, receipt, fleet
management, reports, Swagger UI), §4.2 (`./mvnw test` passing), and §5
(GitHub repository, commit history, Actions CI run). Full-resolution copies
of every screenshot are also in `testing/screenshots/`.
